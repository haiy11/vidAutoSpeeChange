...
echo   ^"type^": ^"stdio^",
echo   ^"allowed_origins^": [^"chrome-extension://kmheiofaggdafdoliglicehdmebloipp/^"]
echo }
) > "%json_file%"
...
:: 添加注册表项（当前用户，无需管理员权限）
reg add "HKEY_CURRENT_USER\SOFTWARE\Microsoft\Edge\NativeMessagingHosts\com.dynamic.speed" /ve /t REG_SZ /d "%json_file%" /f >nul
